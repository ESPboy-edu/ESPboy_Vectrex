...
uint8_t ICACHE_RAM_ATTR read8 (uint16_t address)
{
  uint8_t data;
...
static void alg_addline (int32_t x0, int32_t y0, int32_t x1, int32_t y1, uint8_t color)
{
  ...
  if (index >= 0 && index < vector_draw_cnt &&
    ...
    if (vector_draw_cnt >= VECTOR_CNT) return;
...
static void ICACHE_RAM_ATTR alg_sstep (void)
{
